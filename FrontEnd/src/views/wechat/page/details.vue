<template>
  <div class="details-page">
    ...
    <div class="part-one course-form">
      ...
      <mt-field label="导师" placeholder="导师姓名" v-model="course.teacher" :readonly="!course.isowner" disabled></mt-field>
      <mt-field label="电话" placeholder="导师电话" v-model="course.teacher_phone" :readonly="!course.isowner" disabled></mt-field>
        ...
        <div v-if="userType == 'teacher'" class="details-notify">
          ...
          <div v-if="course.status == 2">
            ...
            <mt-cell title="名单" :label="course.student_list">
              <mt-button v-if="course.isowner" type="primary" @click="pickerStudent">查看</mt-button>
            </mt-cell>
            <div v-if="course.isowner" style="float: right;margin-top:10px">
              ...
              <mt-button type="primary" class="details-notify-btn" @click="" size="normal">处理</mt-button>
          ...
          <div v-if="course.status == 3">
            ...
            <mt-button type="primary" class="details-notify-btn" size="normal">处理</mt-button>
          </div>
        </div>
    ...
    methods: {
      ...
      deleteCourse() {
        util.box.confirm('确定删除此课程?').then(action => {
          this.$http.get("detail/delete?id=" + util.hashArr(3)).then((res) => {
            util.toast({
              message: res.data.msg,
              iconClass: 'mintui mintui-success'
            });
